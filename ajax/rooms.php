...
if (isset($_GET['fetch_rooms'])) {
    ...
    if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
        ...
        if ($checkin_date == $checkout_date) {
            echo "<h3 class='text-center text-danger'>Check-in And Check-out Date Cannot Be Same!</h3>";
            exit;
        } else if ($checkout_date < $checkin_date) {
            echo "<h3 class='text-center text-danger'>Check-out Date Cannot Be Lesser Than Check-in Date!</h3>";
            exit;
        } else if ($checkin_date < $today_date) {
            echo "<h3 class='text-center text-danger'>Check-in Date Cannot Be Lesser Than Today!</h3>";
            exit;
        }
    }
    ...
    while ($room_data = mysqli_fetch_assoc($room_res)) {
        ...
        $fac_q = mysqli_query($con, "SELECT f.name, f.id FROM `facilities` f INNER JOIN
         `room_facilities` rfac ON f.id = rfac.facilities_id WHERE rfac.room_id = '$room_data[id]'");
                    ...
                    <div class='card mb-4 border-0 shadow'>
                        <div class='row g-0 p-3 align-items-center'>
                            ...
                            <div class='col-md-5 px-lg-3 px-md-3 px-0'>
                                ...
                                <div class='feratures mb-3'>
                                    <h6 class='mb-1'>Features</h6>
                                    $features_data
                                </div>
                                <div class='faciliteies mb-3'>
